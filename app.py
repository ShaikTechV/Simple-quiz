...
FRESH DATABASE VERSION - Deploy immediately to Heroku
"""
...
def init_database():
    """Initialize database tables - FRESH DATABASE VERSION"""
    try:
        ...
        if conn:
            ...
            cur.execute("DROP TABLE IF EXISTS quiz_sessions CASCADE")
...
def start_quiz():
    try:
        ...
        if conn:
            ...
            print(f"✅ CREATED QUIZ SESSION: {access_code}")
...
def submit_answer():
    try:
        ...
        print(f"💾 SAVING ANSWER: {access_code} - Q{question_id}: {answer}")
...
def submit_quiz():
    try:
        ...
        print(f"📤 SUBMITTING QUIZ: {access_code}")
